...
func ProcessEvents(labelPrefix string, host string, port int, processorName string, projection interface{}, prepareUnmarshal func (payloadType string)Event) *ClientConnection {
    ...
    if e := registerProcessor(label, processorName, stream, clientConnection.ClientInfo); e != nil {
...
func registerProcessor(label string, processorName string, stream *axon_server.PlatformService_OpenStreamClient, clientInfo *axon_server.ClientIdentification) error {
    ...
    if e != nil {
        log.Printf("%v: Error sending registration", label, e)
        return e
    }
